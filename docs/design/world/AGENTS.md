...
| `VISUAL_IDENTITY.md` | **Consolidated visual reference** — all rendering art direction in one place | Art direction principles, faction PBR specs (per-faction metalness/roughness/emissive/colors), cube material visual table (all 15 types), ore deposit visuals (per-ore geometry and color), environment visuals (storm sky, terrain, biome colors, hazard descriptions), building PBR treatments, special shader docs (holographic shader, fog of war, normal map composer), alien visuals (Ferrovore crystal biology, Residual ancient-metal aesthetics), performance notes (instanced cubes, quality tiers), implementation status table. |
