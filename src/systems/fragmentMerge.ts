...
import { MapFragment, WorldPosition } from "../ecs/traits";
...
export function fragmentMergeSystem(): MergeEvent[] {
	...
	for (let i = 0; i < unitList.length; i++) {
		for (let j = i + 1; j < unitList.length; j++) {
			...
			if (a.get(MapFragment)?.fragmentId === b.get(MapFragment)?.fragmentId)
				continue;

			const dx = a.get(WorldPosition)?.x - b.get(WorldPosition)?.x;
			const dz = a.get(WorldPosition)?.z - b.get(WorldPosition)?.z;
...
function mergeFragments(
	...
	const fragA = getFragment(unitA.get(MapFragment)?.fragmentId);
	const fragB = getFragment(unitB.get(MapFragment)?.fragmentId);
	...
	for (const entity of units) {
		if (entity.get(MapFragment)?.fragmentId === absorbed.id) {
			entity.get(MapFragment)!.fragmentId = survivor.id;
		}
	}
