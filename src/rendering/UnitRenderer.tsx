...
import { getLODLevel, type BotLODLevel } from "../systems/botLOD";
 ...
 */
function normalizeCultistMaterial(
	material: THREE.Material,
) {
	if (!(material instanceof THREE.MeshStandardMaterial)) {
		return;
...
function UnitMesh({ entity }: { entity: UnitEntity }) {
	...
	useFrame(({ clock }, delta) => {
		...
		if (isCultist && groupRef.current) {
			...
			const jitterX =
				flicker > 0.7
					? CULTIST_GLITCH_AMPLITUDE * Math.sin(t * 47)
					: 0;
			const jitterZ =
				flicker > 0.7
					? CULTIST_GLITCH_AMPLITUDE * Math.cos(t * 51)
					: 0;
			groupRef.current.position.x += jitterX;
			groupRef.current.position.z += jitterZ;
			for (const mat of mats) {
				mat.emissiveIntensity =
					0.25 + 0.2 * (0.5 + 0.5 * Math.sin(t * 2.1));
			}
		}
			...
			{buildingType === "fabrication_unit" && (
				<>
					<mesh position={[0, 0.7, 0]} castShadow receiveShadow>
						...
						<meshStandardMaterial color={isPowered ? COLOR_FABRICATION : 0x554433} roughness={0.72} metalness={0.2} emissive={isPowered ? 0x2a1e0e : 0x0a0806} emissiveIntensity={isPowered ? 0.16 : 0.04} />
					</mesh>
					<mesh position={[0, 1.3, 0]}>
						<cylinderGeometry args={[0.08, 0.08, 0.5, 8]} />
						<meshStandardMaterial color={0x666666} roughness={0.78} metalness={0.22} />
					</mesh>
					<mesh position={[0.5, 0.9, 0.61]}>
						<sphereGeometry args={[0.08, 8, 8]} />
						<meshStandardMaterial color={isPowered ? 0x00ff00 : COLOR_BROKEN} emissive={isPowered ? 0x00ff00 : COLOR_BROKEN} emissiveIntensity={0.8} roughness={0.2} metalness={0.05} />
					</mesh>
					<mesh position={[0, 1.2, 0.4]} castShadow>
						<boxGeometry args={[0.4, 0.3, 0.3]} />
						<meshStandardMaterial color={0x777766} roughness={0.74} metalness={0.16} emissive={0x0e0e0a} emissiveIntensity={0.08} />
					</mesh>
				</>
			)}
			...
			{buildingType === "lightning_rod" && (
				<>
					<mesh position={[0, 1.5, 0]} castShadow>
						...
						<meshStandardMaterial color={0x6e7080} roughness={0.58} metalness={0.42} emissive={0x0e1018} emissiveIntensity={0.08} />
					</mesh>
					<mesh position={[0, 2.8, 0]}>
						<coneGeometry args={[0.12, 0.4, 6]} />
						<meshStandardMaterial color={0x99aa22} emissive={0x334400} emissiveIntensity={0.35} roughness={0.32} metalness={0.18} />
					</mesh>
			...
			{buildingType === "defense_turret" && (
				<>
					<mesh position={[0, 0.55, 0]} castShadow>
						...
						<meshStandardMaterial color={isPowered ? 0x6a7080 : 0x4a4a50} roughness={0.6} metalness={0.35} emissive={isPowered ? 0x0e1218 : 0x060608} emissiveIntensity={0.1} />
					</mesh>
					{/* Barrel */}
					<mesh position={[0, 0.7, 0.5]} rotation={[Math.PI / 2, 0, 0]} castShadow>
						<cylinderGeometry args={[0.06, 0.08, 0.7, 6]} />
						<meshStandardMaterial color={0x555560} roughness={0.5} metalness={0.5} />
					</mesh>
					{/* Muzzle tip */}
					<mesh position={[0, 0.7, 0.85]}>
						<sphereGeometry args={[0.06, 6, 6]} />
						<meshStandardMaterial color={isPowered ? 0xff4444 : 0x444444} emissive={isPowered ? 0xff2222 : 0x000000} emissiveIntensity={isPowered ? 0.5 : 0} roughness={0.3} metalness={0.1} />
					</mesh>
				</>
			)}
			...
			{buildingType === "relay_tower" && (
				<>
					<mesh position={[0, 1.2, 0]} castShadow>
						...
						<meshStandardMaterial color={0x7080a0} roughness={0.55} metalness={0.4} />
					</mesh>
					{/* Signal dish */}
					<mesh position={[0.3, 1.8, 0]} rotation={[0, 0, -Math.PI / 6]}>
						<coneGeometry args={[0.25, 0.15, 8, 1, true]} />
						<meshStandardMaterial color={isPowered ? 0x8be6ff : 0x445566} emissive={isPowered ? 0x2266aa : 0x000000} emissiveIntensity={isPowered ? 0.3 : 0} roughness={0.4} metalness={0.3} side={THREE.DoubleSide} />
					</mesh>
					{/* Blinking tip */}
					<mesh position={[0, 2.3, 0]}>
						<sphereGeometry args={[0.06, 6, 6]} />
						<meshStandardMaterial color={isPowered ? 0x00ccff : 0x334455} emissive={isPowered ? 0x00aaff : 0x000000} emissiveIntensity={isPowered ? 0.7 : 0} roughness={0.2} metalness={0.1} />
					</mesh>
				</>
			)}
			...
			{buildingType === "power_sink" && (
				<>
					<mesh position={[0, 0.7, 0]} castShadow receiveShadow>
						...
						<meshStandardMaterial color={isPowered ? 0x556688 : 0x3a3a44} roughness={0.65} metalness={0.3} emissive={isPowered ? 0x112244 : 0x000000} emissiveIntensity={isPowered ? 0.15 : 0} />
					</mesh>
					{/* Coil rings */}
					<mesh position={[0, 1.2, 0]} rotation={[Math.PI / 2, 0, 0]}>
						<torusGeometry args={[0.35, 0.04, 8, 16]} />
						<meshStandardMaterial color={isPowered ? 0xf6c56a : 0x665544} emissive={isPowered ? 0xaa6622 : 0x000000} emissiveIntensity={isPowered ? 0.35 : 0} roughness={0.4} metalness={0.4} />
					</mesh>
					<mesh position={[0, 1.0, 0]} rotation={[Math.PI / 2, 0, 0]}>
						<torusGeometry args={[0.4, 0.03, 8, 16]} />
						<meshStandardMaterial color={isPowered ? 0xddaa44 : 0x554433} roughness={0.5} metalness={0.3} />
					</mesh>
				</>
			)}
			...
			{buildingType === "storage_hub" && (
				<>
					<mesh position={[0, 0.55, 0]} castShadow receiveShadow>
						...
						<meshStandardMaterial color={0x6a7766} roughness={0.8} metalness={0.12} emissive={0x0a0e08} emissiveIntensity={0.06} />
					</mesh>
					{/* Stacked crate */}
					<mesh position={[0.2, 1.0, 0.1]} castShadow>
						<boxGeometry args={[0.8, 0.4, 0.6]} />
						<meshStandardMaterial color={0x7a8866} roughness={0.75} metalness={0.1} />
					</mesh>
					{/* Small side crate */}
					<mesh position={[-0.4, 0.55, 0.35]} castShadow>
						<boxGeometry args={[0.4, 0.3, 0.3]} />
						<meshStandardMaterial color={0x667755} roughness={0.82} metalness={0.08} />
					</mesh>
				</>
			)}
			...
			{buildingType === "habitat_module" && (
				<>
					<mesh position={[0, 0.8, 0]} castShadow receiveShadow>
						...
						<meshStandardMaterial color={isPowered ? 0x8899aa : 0x556666} roughness={0.7} metalness={0.2} emissive={isPowered ? 0x112233 : 0x000000} emissiveIntensity={isPowered ? 0.12 : 0} />
					</mesh>
					{/* Viewport window */}
					<mesh position={[0, 0.9, 0.6]}>
						<circleGeometry args={[0.15, 12]} />
						<meshStandardMaterial color={isPowered ? 0xaaeeff : 0x445566} emissive={isPowered ? 0x44aacc : 0x000000} emissiveIntensity={isPowered ? 0.4 : 0} roughness={0.15} metalness={0.05} transparent opacity={0.85} />
					</mesh>
				</>
			)}
			...
			{buildingType === "motor_pool" && (
				<>
					<mesh position={[0, 0.65, 0]} castShadow receiveShadow>
						...
						<meshStandardMaterial color={isPowered ? 0x7a8090 : 0x505558} roughness={0.7} metalness={0.25} emissive={isPowered ? 0x0e1420 : 0x040406} emissiveIntensity={isPowered ? 0.1 : 0.03} />
					</mesh>
					{/* Bay door */}
					<mesh position={[0, 0.5, 0.61]}>
						<boxGeometry args={[0.8, 0.6, 0.04]} />
						<meshStandardMaterial color={isPowered ? 0x556070 : 0x3a3e44} roughness={0.6} metalness={0.3} />
					</mesh>
					{/* Status strip above door */}
					<mesh position={[0, 0.85, 0.62]}>
						<boxGeometry args={[0.9, 0.06, 0.02]} />
						<meshStandardMaterial color={isPowered ? 0x00ccaa : 0x333333} emissive={isPowered ? 0x00aa88 : 0x000000} emissiveIntensity={isPowered ? 0.6 : 0} roughness={0.3} metalness={0.1} />
					</mesh>
				</>
			)}
