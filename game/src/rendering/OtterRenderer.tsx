...
function OtterMesh({ entity }: { entity: OtterEntity }) {
  ...
  useFrame((state) => {
    ...
    if (leftFrontPawRef.current) leftFrontPawRef.current.position.z = 0.13 + stride * 0.04
    if (rightFrontPawRef.current) rightFrontPawRef.current.position.z = 0.13 - stride * 0.04
    if (leftBackPawRef.current) leftBackPawRef.current.position.z = -0.12 - stride * 0.04
    if (rightBackPawRef.current) rightBackPawRef.current.position.z = -0.12 + stride * 0.04
  })
