...
export function otterSystem() {
  for (const entity of otters) {
    ...
    const clampedX = Math.max(-WORLD_HALF + OTTER_BORDER, Math.min(WORLD_HALF - OTTER_BORDER, newX))
    const clampedZ = Math.max(-WORLD_HALF + OTTER_BORDER, Math.min(WORLD_HALF - OTTER_BORDER, newZ))
