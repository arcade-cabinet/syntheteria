...
function UnitRow({
	...
	return (
		<button
			onClick={() => onPress(unit.position)}
			data-testid={`roster-unit-${unit.id}`}
			aria-label={`${unit.displayName}, ${roleInfo.label}, Mark ${unit.markLevel}, ${activityInfo.label} at ${gridX},${gridZ}`}
			style={{
				display: "flex",
				flexDirection: "row",
				alignItems: "center",
				...
				border: `1px solid ${isSpent ? "rgba(255, 255, 255, 0.06)" : "rgba(126, 231, 203, 0.12)"}`,
