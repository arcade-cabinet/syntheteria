# City & World Contracts

Canonical contract layer for world/city session state, city kit packages, and coordination guidance for agents working on TSX surfaces.
...
If a UI surface needs a world/city session shape, prefer importing the shared snapshot type.
...
- terrain atlas / hex-tile contract validation for the world tilesets
...
- `src/config/terrainAtlasContracts.test.ts`
...
- Avoid introducing duplicate session or nearby-POI shapes in TSX while these contracts are stabilizing.
...
- `CitySiteModal.tsx` and `LocationPanel.tsx` should treat `src/world/citySiteActions.ts` and `src/world/locationContext.ts` as the source of truth for interaction logic.
