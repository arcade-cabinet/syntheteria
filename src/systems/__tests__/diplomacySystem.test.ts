...
// ---------------------------------------------------------------------------
// diplomacySystem tick — opinion decay only
// Note: AI trade proposals are generated by aiCivilizationSystem (which has
// access to real faction resource states). diplomacySystem only decays opinions.
// ---------------------------------------------------------------------------

describe("diplomacySystem tick", () => {
	it("decays opinions at check interval", () => {
		adjustOpinion("reclaimers", "iron_creed", 50);

		diplomacySystem(300);

		// Opinion should have decayed slightly toward 0
		const relation = getRelation("reclaimers", "iron_creed");
		expect(relation.opinion).toBeLessThan(50);
	});

	it("does not decay opinions between check intervals", () => {
		adjustOpinion("reclaimers", "iron_creed", 50);

		diplomacySystem(1); // not on interval

		const relation = getRelation("reclaimers", "iron_creed");
		expect(relation.opinion).toBe(50);
	});

	it("does not generate trade proposals (that is aiCivilizationSystem's job)", () => {
		adjustOpinion("reclaimers", "iron_creed", 35);

		diplomacySystem(300);

		// diplomacySystem no longer calls aiProposeTrades
...
// ---------------------------------------------------------------------------
// Event bus integration — diplomacy_changed events
// ---------------------------------------------------------------------------

describe("diplomacy_changed events", () => {
	it("emits diplomacy_changed when stance crosses neutral→friendly threshold", () => {
		// neutral (0) → friendly (30)
		adjustOpinion("player", "reclaimers", 30, 42);
	...
	it("emits diplomacy_changed when stance crosses friendly→allied threshold", () => {
		adjustOpinion("player", "reclaimers", 30, 0); // neutral → friendly
		mockEmit.mockClear();

		adjustOpinion("player", "reclaimers", 30, 100); // friendly → allied

		expect(mockEmit).toHaveBeenCalledTimes(1);
		expect(mockEmit).toHaveBeenCalledWith(
			expect.objectContaining({
				type: "diplomacy_changed",
				previousStance: "friendly",
				newStance: "allied",
				tick: 100,
			}),
		);
	...
	it("emits diplomacy_changed when stance crosses neutral→unfriendly threshold", () => {
		adjustOpinion("player", "volt_collective", -20, 5);
		...
		expect(mockEmit).toHaveBeenCalledWith(
			expect.objectContaining({
				type: "diplomacy_changed",
				previousStance: "neutral",
				newStance: "unfriendly",
			}),
		);
	});

	it("emits diplomacy_changed when stance crosses to hostile", () => {
		adjustOpinion("player", "volt_collective", -50, 10);
	...
	it("does NOT emit event when opinion changes without crossing a stance threshold", () => {
		// Small adjustment stays within neutral range
		adjustOpinion("player", "reclaimers", 10, 0); // still neutral
		expect(mockEmit).not.toHaveBeenCalled();
	});

	it("does NOT emit event when opinion stays at 0", () => {
		adjustOpinion("player", "reclaimers", 0, 0);
		expect(mockEmit).not.toHaveBeenCalled();
	});

	it("emits event via modifyOpinion when stance changes", () => {
		// attackedUs = -40: neutral → unfriendly
		modifyOpinion("reclaimers", "iron_creed", "attackedUs", 77);

		expect(mockEmit).toHaveBeenCalledTimes(1);
		expect(mockEmit).toHaveBeenCalledWith(
			expect.objectContaining({
				type: "diplomacy_changed",
				factionA: expect.any(String),
				factionB: expect.any(String),
				previousStance: "neutral",
				newStance: "unfriendly",
				tick: 77,
			}),
	...
	it("factionA and factionB in event match the input factions (either order)", () => {
		adjustOpinion("iron_creed", "signal_choir", 30, 0);

		expect(mockEmit).toHaveBeenCalledWith(
			expect.objectContaining({
				type: "diplomacy_changed",
				factionA: expect.stringMatching(/iron_creed|signal_choir/),
				factionB: expect.stringMatching(/iron_creed|signal_choir/),
			}),
		);
	});

	it("does not emit on decayOpinions when stance does not change", () => {
		adjustOpinion("player", "reclaimers", 10, 0); // stays neutral — no event
		mockEmit.mockClear();

		decayOpinions(); // 10 → ~9.9, still neutral
