...
const STORAGE_KEY = "syntheteria:fallback-database";
	...
	private loadFromStorage() {
		...
		try {
			const snapshot = JSON.parse(raw) as {
				nextId: number;
				saveGames: SaveGameRow[];
				ecumenopolisMaps: EcumenopolisMapRow[];
				sectorCells: SectorCellRow[];
				sectorStructures: SectorStructureRow[];
				worldPois: WorldPoiRow[];
				cityInstances: CityInstanceRow[];
			...
			this.saveGames = snapshot.saveGames;
			this.ecumenopolisMaps = snapshot.ecumenopolisMaps;
			this.sectorCells = snapshot.sectorCells;
			this.sectorStructures = snapshot.sectorStructures ?? [];
			this.worldPois = snapshot.worldPois;
			this.cityInstances = snapshot.cityInstances;
	...
	persistToStorage() {
		...
		window.localStorage.setItem(
			...
			JSON.stringify({
				...
				ecumenopolisMaps: this.ecumenopolisMaps,
				sectorCells: this.sectorCells,
				sectorStructures: this.sectorStructures,
				worldPois: this.worldPois,
				cityInstances: this.cityInstances,
