...
import groundMaterialsJson from "../config/groundMaterials.json";
...
// ─── Ground materials (infinite grid floor) ─────────────────────────────────

interface GroundMaterialDef {
	id: string;
	zoneType: string;
	baseColorHex: string;
	textures: Record<string, string>;
...
function seedGroundMaterials(database: SyncDatabase): void {
	const materials = (
		groundMaterialsJson as { materials: GroundMaterialDef[] }
	).materials;
