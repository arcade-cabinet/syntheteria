 ...
 *   - Globe: Earth continents → ecumenopolis city spread with cyan/mint lights
...
export const globeFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // City growth: starts from land, then spreads to ocean platforms
    float cityPattern = fbm(pos * 6.0 + uTime * 0.03);
    float growth = smoothstep(0.0, 1.0, uGrowth);

    float citySpread = landMask * growth * 2.0;
    citySpread += (1.0 - landMask) * max(0.0, growth * 2.0 - 1.0);
    citySpread = clamp(citySpread, 0.0, 1.0);

    float cityMask = smoothstep(0.3, 0.7, cityPattern + citySpread * 0.5) * citySpread;
    ...
    // City colors: the ecumenopolis palette
    vec3 cityBase = vec3(0.09, 0.10, 0.13);
    vec3 cityDense = vec3(0.14, 0.16, 0.21);
    vec3 cityCore = vec3(0.20, 0.25, 0.35);
    vec3 cityGlow = vec3(0.35, 0.75, 0.88);  // cyan-mint city lights
    ...
    vec3 urbanColor = mix(cityBase, cityDense, cityPattern);
    urbanColor = mix(urbanColor, cityCore, smoothstep(0.5, 1.0, cityMask));

    vec3 color = mix(terrainColor, urbanColor, cityMask);

    // City lights: pulsing grid pattern in dense areas
    float lightGrid = fbm(pos * 40.0 + uTime * 0.2);
    float lights = cityMask * smoothstep(0.4, 0.6, lightGrid);
    lights *= (0.5 + 0.5 * sin(uTime * 2.0 + cityPattern * 30.0));
    lights *= smoothstep(0.3, 0.8, cityMask);

    color += cityGlow * lights * 0.6;

    // Atmospheric fresnel glow — cyan/signal color at edges
    float fresnel = pow(1.0 - abs(dot(vNormal, vec3(0.0, 0.0, 1.0))), 3.0);
    vec3 atmosColor = mix(vec3(0.04, 0.12, 0.25), vec3(0.12, 0.22, 0.32), cityMask);
    color += atmosColor * fresnel * 0.5;

    // Cloud shadows
    float clouds = fbm(pos * 10.0 + uTime * 0.1) * 0.1;
    color *= (1.0 - clouds * (1.0 - cityMask * 0.8));
