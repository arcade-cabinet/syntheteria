...
export interface CityPurposePresentation {
	badge: string;
	enterLabel: string;
...
const PRESENTATION: Record<WorldPoiType, CityPurposePresentation> = {
	home_base: {
		badge: "Home Base",
	...
	northern_cult_site: {
		...
		summary:
			"This site marks cult-controlled territory. It should read as dangerous and inaccessible until combat and faction systems mature.",
	...
	deep_sea_gateway: {
		...
		summary:
			"The gateway remains mechanically reserved for later campaign phases, but its spatial contract is already persisted and visible in the world.",
		surveyLabel: "Scan Gateway Anchor",
	},
};

export function getCityPurposePresentation(poiType: WorldPoiType) {
	return PRESENTATION[poiType];
}

export function describeCityState(state: CityInstanceState | undefined) {
	switch (state) {
		case "latent":
